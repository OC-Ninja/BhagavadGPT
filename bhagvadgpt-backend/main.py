...
from langchain_core.messages import SystemMessage, HumanMessage, AIMessage  # Phase 2: AIMessage for history
...
async def openai_adapter(request: Request):
    try:
        ...
        user_message = all_messages[-1]["content"]  # current query — used for DB search

        username = data.get("user", "") if data.get("user") else "Friend"

        # 1. Search DB (always uses current message only — history irrelevant for retrieval)
        ...
        # 2. Role-separated messages + Phase 2: 1 exchange history (safe/token-efficient)
        system_content = SYSTEM_PROMPT.replace("{context}", context_str).replace("{username}", username)

        # Build history: only last assistant message if it exists
        # 1 exchange = last assistant response only
        # Keeps token cost minimal for multi-user free tier
        history = []
        if len(all_messages) >= 2:
            prev = all_messages[-2]
            if prev.get("role") == "assistant":
                history.append(AIMessage(content=prev["content"]))

        try:
            response = llm.invoke([
                SystemMessage(content=system_content),
                *history,                           # ← previous assistant response (if any)
                HumanMessage(content=user_message)  # ← current user message isolated
            ])
